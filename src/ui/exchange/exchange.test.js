...
describe('<ExchangeComponent />', () => {
    ...
    beforeEach(() => {
        wrapper = mount(<ExchangeComponent {...data} />);
    });
    it('input fields', () => {
        expect(
            wrapper
    ...
    it('balance fields', () => {
        expect(
            wrapper
                ...
                .at(0)
                .text()
        ).toBe(`Balance ${data.balanceFrom}`);
        expect(
            wrapper
                .find('.exchange__slot_balance')
                .at(1)
                .text()
        ).toBe(`Balance ${data.balanceTo}`);
    });
    it('rate field', () => {
        expect(wrapper.find('.exchange__actions_currency-rate').text()).toBe(
            `${data.rateFrom} = ${data.rateTo}`
        );
    });
    it('currency fields', () => {
        expect(
            wrapper
                ...
                .at(0)
                .text()
        ).toBe(data.currencyFrom);
        expect(
            wrapper
                .find('.exchange__slot_currency')
                .at(1)
                .text()
        ).toBe(data.currencyTo);
    });
    it('exchange error', () => {
        expect(wrapper.find('.exchange__error').text()).toBe(
            data.exchangeRatesError
        );
    });
